...
Use this as a script to run the demo live in front of your team. It
assumes the pages are open at:

- `index.html` — landing page
- `vulnerable.html` — vulnerable demo
- `patched.html` — patched demo
- `clickjack-test.html` — side-by-side framing test
...
**Open:** `vulnerable.html`, section 1 (Login)
...
**Then open** `patched.html` and repeat the exact same input. Show that
it returns "Invalid credentials" — the input is compared as a plain
value, never interpreted as part of a command, so the injection has no
effect.
...
**Open:** `vulnerable.html`, section 2 (Search)
...
**Then open** `patched.html` and paste the same payload into search.
**Result:** the tag is displayed as plain visible text, not executed —
no alert box. Explain: the output is now inserted as text content, not
parsed as HTML, so markup characters have no special meaning.
...
1. Point out the two panels: the vulnerable page loads normally inside
   the iframe on the left. The patched page on the right immediately
   detects it's being framed and blocks its own content.
